...
void report_hotspot(const TimingResult* results, int n)
{
    ...
    double hotspot_fraction = (total > 0.0)
                            ? (results[max_idx].time_us / total)
                            : 0.0;
    printf("\n>> Hotspot Analysis\n");
    ...
    double amdahl = (hotspot_fraction < 1.0)
                  ? (1.0 / (1.0 - hotspot_fraction))
                  : 9999.0;
...
void report_optimization_sweep(const SweepResult* sweep, int n, const char* out_path)
{
    ...
    printf("\n%-8s %12s %10s %10s %10s %12s %10s\n",
           "Flag", "Gaussian", "Sobel", "Mag", "Dir", "Total(us)", "Size(KB)");
    printf("%-8s %12s %10s %10s %10s %12s %10s\n",
           "----", "--------", "-----", "---", "---", "---------", "--------");
    ...
    int best_size_idx = 0;
    for (int i = 0; i < n; i++) {
        if (sweep[i].total_us    < sweep[best_time_idx].total_us) best_time_idx = i;
        if (sweep[i].binary_kb   < sweep[best_size_idx].binary_kb) best_size_idx = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        printf("%-8s %11.2f %10.2f %10.2f %10.2f %12.2f %9ld %s\n",
               sweep[i].flag,
    ...
    fprintf(f, "%-8s %12s %10s %10s %10s %12s %10s\n",
            "Flag", "Gaussian", "Sobel", "Mag", "Dir", "Total(us)", "Size(KB)");
    fprintf(f, "%-8s %12s %10s %10s %10s %12s %10s\n",
            "----", "--------", "-----", "---", "---", "---------", "--------");
    for (int i = 0; i < n; i++) {
        ...
        fprintf(f, "%-8s %11.2f %10.2f %10.2f %10.2f %12.2f %9ld %s\n",
                sweep[i].flag,
                        ...
                        const char* out_path)
{
    ...
    printf("\n%-20s %12s %12s %12s %12s %12s\n",
           "Stage", "Scalar(us)", "RVV-128(us)", "RVV-256(us)", "RVV-512(us)", "Speedup-512");
    ...
    for (int i = 0; i < n_stages; i++) {
        double speedup = (rvv[2][i].time_us > 0.0)
                       ? (scalar[i].time_us / rvv[2][i].time_us)
                       : 0.0;
    ...
    double overall_speedup = (total_rvv512 > 0.0)
                           ? (total_scalar / total_rvv512)
                           : 0.0;
    ...
    fprintf(f, "%-20s %12.2f %12s %12s %12.2f %11.2fx\n",
            "TOTAL", total_scalar, "-", "-", total_rvv512, os);
    fclose(f);
